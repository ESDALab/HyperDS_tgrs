...
class Decoder(nn.Module):
    ...
    def forward(self, x,q, attn_mask=None):
        ...
        if self.conv_layers is not None:
            for attn_layer, conv_layer in zip(self.attn_layers, self.conv_layers):
                x, attn = attn_layer(x,q, attn_mask=attn_mask)
                x = conv_layer(x,q)
                attns.append(attn)
            x, attn = self.attn_layers[-1](x, attn_mask=attn_mask)
            attns.append(attn)
...
class TransformerNet(nn.Module):
    def __init__(self,  c_out,
                d_model=512, n_heads=8, e_layers=2,d_layers=2, d_ff=512,
                 activation='gelu',
                 output_attention=False,**kwargs):
    ...
    def forward(self, x_enc,q_enc,
                enc_self_mask=None):
        # enc_out = self.enc_embedding(x_enc)
        enc_out, attns = self.encoder(x_enc, attn_mask=enc_self_mask)
        # enc_out=enc_out[:,0:data_idx]
        dec_out,attns=self.decoder(enc_out,q_enc)
